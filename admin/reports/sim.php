...
$where[] = "DATE(sc.transaction_date) >= :today";
$params[':today'] = date('Y-m-d'); // Only current and future

if ($start_date) {
    $where[] = "DATE(sc.transaction_date) >= :start_date";
    $params[':start_date'] = $start_date;
}
if ($end_date) {
    $where[] = "DATE(sc.transaction_date) <= :end_date";
...
try {
    ...
    $sql = "
        ...
        $whereSQL
        ORDER BY sc.transaction_date ASC, sc.id ASC
    ";
    $stmt = $db->prepare($sql);
    $stmt->execute($params);
    $sim = $stmt->fetchAll(PDO::FETCH_ASSOC);

    $total = count($sim);
    $total_qty = 0;
    foreach ($sim as $row) $total_qty += (int)$row['quantity_received'];
...
<head>
    ...
    <title>SIM Card Activities Report</title>
...
<body>
    ...
    <nav class="sidebar-nav d-none d-lg-block">
        ...
        <ul class="nav flex-column">
            ...
            <li class="nav-item"><a class="nav-link active" href="sim.php"><i class="bi bi-sim me-2"></i>SIM Card Activity</a></li>
    ...
    <div class="main-content">
        ...
        <div class="container-fluid mt-4">
            <h3 class="mb-3"><i class="bi bi-sim me-2"></i>SIM Card Activities & Transactions (Current & Future)</h3>
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">SIM Cards by Branch & Staff</h5>
                    <div>
                        <button class="btn btn-success me-2" onclick="exportTableToExcel('simTable', 'sim_report.xlsx')">
                ...
                <div class="card-body" id="reportContent">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-4">
                            <div class="stats-card text-center">
                                ...
                                <small class="text-success">Sum of SIMs received</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover" id="simTable">
                            <thead>
                                <tr>
                                    <th>SIM ID</th>
                                    <th>Date</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td><?php echo htmlspecialchars($row['sim_id']); ?></td>
                                ...
                                <?php endforeach; ?>
                                <?php if (empty($sim)): ?>
                                    <tr><td colspan="13" class="text-center text-muted">No SIM activities found for current/future filter.</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
    ...
    function exportTableToExcel(tableID, filename = ''){
        ...
        filename = filename ? filename : 'sim_report.xls';
