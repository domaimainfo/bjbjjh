...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($staff_id <= 0 || $sell_amount < 0) {
        ...
        $stmt = $conn->prepare("SELECT IFNULL(SUM(quantity),0) FROM sim_cards_staff_allocations WHERE branch_id = ? AND staff_id = ? AND allocation_date <= ?");
        ...
        $stmt = $conn->prepare("SELECT closing FROM staff_sim_cards_sales WHERE branch_id = ? AND staff_id = ? AND sell_date < ? ORDER BY sell_date DESC, id DESC LIMIT 1");
        $stmt->execute([$branch_id, $staff_id, $sell_date]);
        ...
        if ($actual_value > $total_balance) {
            ...
            // Insert staff sim sale
            $stmt = $conn->prepare("INSERT INTO staff_sim_cards_sales (branch_id, staff_id, sell_date, receive, opening, total_balance, sell, actual_value, closing) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            <div class="card-body">
                <form method="post" autocomplete="off">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="1" min="0" id="sell" name="sell" class="form-control" value="<?= htmlspecialchars($_POST['sell'] ?? '') ?>" required>
