...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        ");
        $stmt->execute([
            $branch_id, $staff_id, $bank_account_id, $deposit_date, $notes_2000, $notes_500, $notes_200, $notes_100,
...
<head>
    ...
    <style>
        .form-label { font-weight: 500; margin-bottom: 0.2rem; }
        .uniform-box {
            background: #fff;
                        ...
                        for($i=0; $i<count($notes); $i+=2): ?>
                            <div class="denomination-row">
                                <div class="denomination-box">
                                    ...
                                    <input type="number" class="form-control" name="notes_<?php echo $notes[$i]; ?>" id="notes_<?php echo $notes[$i]; ?>" min="0" value="0" oninput="calculateTotal()">
                                </div>
                                <?php if(isset($notes[$i+1])): ?>
                                <div class="denomination-box">
                                    <label class="form-label mb-0" for="notes_<?php echo $notes[$i+1]; ?>">₹<?php echo $notes[$i+1]; ?></label>
                                    <input type="number" class="form-control" name="notes_<?php echo $notes[$i+1]; ?>" id="notes_<?php echo $notes[$i+1]; ?>" min="0" value="0" oninput="calculateTotal()">
                                </div>
                                <?php endif; ?>
                            </div>
                        <?php endfor; ?>
