...
// Get today's apb transaction (for that date)
...
$stmt = $conn->prepare("SELECT IFNULL(SUM(quantity),0) FROM apb_staff_allocations WHERE branch_id = ? AND allocation_date = ?");
$stmt->execute([$branch_id, $allocation_date]);
...
// Insert allocation
$stmt = $conn->prepare("INSERT INTO apb_staff_allocations (branch_id, staff_id, allocation_date, quantity, notes) VALUES (?, ?, ?, ?, ?)");
