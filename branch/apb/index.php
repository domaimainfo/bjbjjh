...
// Today's APB transaction (if any)
$stmt = $conn->prepare("
    SELECT a.*, 
    ...
    LEFT JOIN bank_accounts bacc ON a.bank_account_id = bacc.id
    LEFT JOIN branches br ON a.branch_id = br.id
    WHERE a.branch_id = ? AND DATE(a.transaction_date) = ?
...
$stmt = $conn->prepare("SELECT IFNULL(SUM(quantity),0) AS allocated FROM apb_staff_allocations WHERE branch_id = ? AND allocation_date = ?");
$stmt->execute([$branch_id, $today]);
$today_allocated = (int)$stmt->fetchColumn();

// Today's staff sell (sum of all staff actual_value for today)
$stmt = $conn->prepare("SELECT IFNULL(SUM(actual_value),0) AS staff_sale FROM staff_apb_sales WHERE branch_id = ? AND sell_date = ?");
$stmt->execute([$branch_id, $today]);
...
// For transactions table: show today's APB transaction if any
$recent_apb = $today_apb ? [$today_apb] : [];
$apb_date = $today;

// For Staff Allocations Table: show all allocations for today
$stmt = $conn->prepare("
    SELECT a.*, s.full_name 
    FROM apb_staff_allocations a
...
<head>
    <meta charset="UTF-8">
    <title>APB Management</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../../assets/css/bootstrap.min.css">
    <link rel="stylesheet" href="../../assets/css/bootstrap-icons.css">
    <style>
        .apb-stats-value { font-size: 1.5rem; font-weight: 700; }
        .apb-stats-label { font-size: .97rem; font-weight: 500; color: #6c757d; }
...
<body class="bg-light">
    ...
    <div class="sidebar d-none d-lg-block">
        ...
        <nav class="nav flex-column">
            ...
            <a class="nav-link<?= (basename($_SERVER['PHP_SELF'])=='index.php') ? ' active' : '' ?>" href="index.php"><i class="bi bi-credit-card"></i> APB</a>
        ...
        <div class="container-fluid py-3">
            ...
            <div class="row g-4 mb-4">
                <div class="col-6 col-md-2">
                    <div class="card h-100 shadow-sm">
                        <div class="card-body text-center">
                            <span class="apb-stats-label">Opening Stock</span>
                            <div class="apb-stats-value"><?= number_format($stats['opening'], 0) ?></div>
                        </div>
                    </div>
                </div>
                <div class="col-6 col-md-2">
                    <div class="card h-100 shadow-sm">
                        <div class="card-body text-center">
                            <span class="apb-stats-label">Received</span>
                            <div class="apb-stats-value"><?= number_format($stats['received'], 0) ?></div>
                        </div>
                    </div>
                </div>
                <div class="col-6 col-md-2">
                    <div class="card h-100 shadow-sm">
                        <div class="card-body text-center">
                            <span class="apb-stats-label">Auto Quantity</span>
                            <div class="apb-stats-value"><?= number_format($stats['auto'], 0) ?></div>
                        </div>
                    </div>
                </div>
                <div class="col-6 col-md-2">
                    <div class="card h-100 shadow-sm">
                        <div class="card-body text-center">
                            <span class="apb-stats-label">Total Available</span>
                            <div class="apb-stats-value"><?= number_format($stats['available'], 0) ?></div>
                        </div>
                    </div>
                </div>
                <div class="col-6 col-md-2">
                    <div class="card h-100 shadow-sm">
                        <div class="card-body text-center">
                            <span class="apb-stats-label">Allocated to Staff</span>
                            <div class="apb-stats-value"><?= number_format($stats['allocated'], 0) ?></div>
                        </div>
                    </div>
                </div>
                <div class="col-6 col-md-2">
                    <div class="card h-100 shadow-sm">
                        <div class="card-body text-center">
                            <span class="apb-stats-label">Total Sale</span>
                            <div class="apb-stats-value"><?= number_format($stats['sale'], 0) ?></div>
                        </div>
                    </div>
                </div>
                <div class="col-6 col-md-2 mt-md-3">
                    <div class="card h-100 shadow-sm">
                        <div class="card-body text-center">
                            <span class="apb-stats-label">Closing Stock</span>
                            <div class="apb-stats-value"><?= number_format($stats['closing'], 0) ?></div>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="card mb-4">
    <div class="card-body">
        ...
        <a href="reports.php" class="btn btn-secondary">
            ...
            <!-- Recent APB Transactions Table (today's, if any) -->
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">
                        Today's APB Transaction (<?= htmlspecialchars($apb_date) ?>)
                    </h5>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover align-middle text-center">
                            ...
                            <tbody>
                                ...
                                <?php endforeach; ?>
                                <?php if (empty($recent_apb)): ?>
                                <tr>
                                    <td colspan="12" class="text-muted">No APB transactions found for today.</td>
                                </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
            ...
            <!-- Staff APB Allocations Table -->
            <div class="card mt-4">
                <div class="card-header">
                    <h5 class="mb-0">Today's APB Allocations to Staff</h5>
                </div>
            ...
            <!-- Staff APB Sell Table -->
            <div class="card mt-4">
                <div class="card-header">
                    <h5 class="mb-0">Today's APB Staff Sales</h5>
                </div>
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-hover align-middle text-center mb-0">
