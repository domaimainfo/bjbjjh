...
$message = '';
$receive = $opening = $total_balance = $sell_amount = $actual_value = $closing = 0;
$staff_id = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sell_amount = intval($_POST['sell'] ?? 0);
    ...
    if ($staff_id <= 0 || $sell_amount < 0) {
        ...
        $stmt = $conn->prepare("SELECT IFNULL(SUM(quantity),0) FROM apb_staff_allocations WHERE branch_id = ? AND staff_id = ? AND allocation_date <= ?");
        $stmt->execute([$branch_id, $staff_id, $sell_date]);
        $receive = intval($stmt->fetchColumn());

        // Get staff's last closing up to before this date
        $stmt = $conn->prepare("SELECT closing FROM staff_apb_sales WHERE branch_id = ? AND staff_id = ? AND sell_date < ? ORDER BY sell_date DESC, id DESC LIMIT 1");
        $stmt->execute([$branch_id, $staff_id, $sell_date]);
        $last_closing = $stmt->fetchColumn();
        $opening = $last_closing !== false ? intval($last_closing) : 0;

        $total_balance = $receive + $opening;

        $actual_value = $sell_amount + ceil($sell_amount * 2.9 / 100); // integer values
        if ($actual_value > $total_balance) {
            ...
            // Insert staff sale
            $stmt = $conn->prepare("INSERT INTO staff_apb_sales (branch_id, staff_id, sell_date, receive, opening, total_balance, sell, actual_value, closing) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$branch_id, $staff_id, $sell_date, $receive, $opening, $total_balance, $sell_amount, $actual_value, $closing]);
            $message = '<div class="alert alert-success">Sale recorded! Staff: ' . htmlspecialchars($staff_id) . ', Sold: ' . number_format($sell_amount, 0) . ', Closing: ' . number_format($closing, 0) . '</div>';
        }
    }
}
?>
            ...
            <div class="card-body">
                <form method="post" autocomplete="off">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" value="<?= number_format($receive, 0) ?>" readonly>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Opening Balance (Staff's last closing)</label>
                        <input type="number" class="form-control" value="<?= number_format($opening, 0) ?>" readonly>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Total Balance</label>
                        <input type="number" class="form-control" value="<?= number_format($total_balance, 0) ?>" readonly>
                    </div>
                    <?php endif; ?>
                    <div class="mb-3">
                        <label class="form-label" for="sell">Sell (input amount)</label>
                        <input type="number" min="0" id="sell" name="sell" class="form-control" value="<?= htmlspecialchars($_POST['sell'] ?? '') ?>" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" value="<?= number_format($actual_value, 0) ?>" readonly>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Closing</label>
                        <input type="number" class="form-control" value="<?= number_format($closing, 0) ?>" readonly>
                    </div>
                    <?php endif; ?>
