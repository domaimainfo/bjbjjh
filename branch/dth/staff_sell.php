...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($staff_id <= 0 || $sell_amount < 0) {
        ...
        $stmt = $conn->prepare("SELECT IFNULL(SUM(quantity),0) FROM dth_staff_allocations WHERE branch_id = ? AND staff_id = ? AND allocation_date <= ?");
        $stmt->execute([$branch_id, $staff_id, $sell_date]);
        $receive = intval($stmt->fetchColumn());

        // Get staff's last closing up to before this date
        $stmt = $conn->prepare("SELECT closing FROM staff_dth_sales WHERE branch_id = ? AND staff_id = ? AND sell_date < ? ORDER BY sell_date DESC, id DESC LIMIT 1");
        $stmt->execute([$branch_id, $staff_id, $sell_date]);
        ...
        $actual_value = $sell_amount + ceil($sell_amount * 2.9 / 100);
        if ($actual_value > $total_balance) {
            ...
            // Insert staff sale
            $stmt = $conn->prepare("INSERT INTO staff_dth_sales (branch_id, staff_id, sell_date, receive, opening, total_balance, sell, actual_value, closing) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
